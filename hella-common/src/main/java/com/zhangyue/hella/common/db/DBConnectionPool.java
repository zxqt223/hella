...
/**
 * @Descriptions The class DBConnectionPool.java's implementation：数据库连接池
 * @author scott 2013-8-19 上午11:04:21
 * @version 1.0
 */
public class DBConnectionPool {
    ...
    private void createConnections(int numConnections) {
        ...
        for (int x = 0; x < numConnections; x++) {
            ...
            if (this.maxConnections > 0 && this.connections.size() >= this.maxConnections) {
                log.error("fail to create db connection,this Pooled Connection size has  exceeds the maxConnections size, refresh Connections......");
                try {
    ...
    private void closeConnection(Connection conn) {
        try {
            ...
            log.error("fail to close connection.The error msg is ： " + e.getMessage());
        }
    }
