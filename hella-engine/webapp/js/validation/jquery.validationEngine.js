	 ...
	 var methods = {
		...
		*/
		attach: function(userOptions) {
			...
			if (options.binded) {
				...
				// bind fields
				form.find("["+options.validateAttribute+"*=validate]").not("[type=checkbox]").not("[type=radio]").not(".datepicker").not(".Wdate").bind(options.validationEventTrigger, methods._onFieldEvent);
				form.find("["+options.validateAttribute+"*=validate][type=checkbox],[class*=validate][type=radio]").not(".Wdate").bind("click", methods._onFieldEvent);
				form.find("["+options.validateAttribute+"*=validate][class*=datepicker]").not(".Wdate").bind(options.validationEventTrigger,{"delay": 300}, methods._onFieldEvent);
			}
			...
			// bind form.submit
			form.bind("checkInput", methods._checkInputEvent);
			return this;
		},
		/**
		...
		*/
		detach: function() {
			...
			// unbind fields
			form.find("["+options.validateAttribute+"*=validate]").not("[type=checkbox]").not(".Wdate").unbind(options.validationEventTrigger, methods._onFieldEvent);
			form.find("["+options.validateAttribute+"*=validate][type=checkbox],[class*=validate][type=radio]").not(".Wdate").unbind("click", methods._onFieldEvent);

			// unbind form.submit
			form.unbind("submit", methods.onAjaxFormComplete);

			// unbind live fields (kill)
			form.find("["+options.validateAttribute+"*=validate]").not("[type=checkbox]").not(".Wdate").die(options.validationEventTrigger, methods._onFieldEvent);
			form.find("["+options.validateAttribute+"*=validate][type=checkbox]").not(".Wdate").die("click", methods._onFieldEvent);
		...
		*/
		...
		_checkInputEvent:function(){
		var form = $(this);
			var options = form.data('jqv');

			// validate each field (- skip field ajax validation, no necessary since we will perform an ajax form validation)
			return methods._validateFields(form, true);			
		},
		/**
		...
		*/
		_required: function(field, rules, i, options) {
			switch (field.prop("type")) {
				...
				case "file":
				default:
					 field.val(methods._ToCDB(field.val()));//全角转半角,去掉前后空格
					if (!field.val())
     		      ...
     		      str= str.replace(/^[\s　\t]+|[\s　\t]+$/, "");
     	           ...
     	           for(var   i=0;i<str.length;i++)  {    
     	                if(str.charCodeAt(i)>65248&&str.charCodeAt(i)<65375){   
     	                   ...
     	                   if(str.charCodeAt(i)==12288){//全角空格字符
		...
		_calculatePosition: function (field, promptElmt, options) {
			...
			if(!methods.isRTL(field))
			{
				switch (positionType) {
					...
					case "bottomRight":
						promptleftPosition += fieldWidth - 30;
						promptTopPosition += field.height() + 45;
						marginTopSize = -(promptTopPosition+5);//add by lxb
				}
			}
			else
			{
		...
		*/
		 _saveOptions: function(form, options) {
			 ...
			 var allRules = $.validationEngineLanguage.allRules;
			     if($.validationEngineLanguage_cycas){
				 allRules =$.extend(true,{}, allRules,$.validationEngineLanguage_cycas.allRules);
				 }
			 }
			 else
