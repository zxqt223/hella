    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        */
        ...
        upgrade: function (rowParm)
        {
            ...
            $("> td > div > .l-grid-tree-space:last", rowobjs).addClass("l-grid-tree-link l-grid-tree-link-close");
        },
        ...
        expand: function (rowParm,loadAjax)
        {
            var g = this, p = this.options;
            var targetRowObj = g.getRowObj(rowParm);
            var linkbtn = $(".l-grid-tree-link", targetRowObj);
            if (linkbtn.hasClass("l-grid-tree-link-open")) return;
            g.toggle(rowParm,loadAjax);
        },
        keepIcon:function(rowParm){
        	 if (!rowParm) return;
             var g = this, p = this.options;
             var rowdata = g.getRow(rowParm);
    		 var subParent = g.getParent(rowdata);
           	 while(subParent!=null){
           		 var subParentRow = $([g.getRowObj(subParent['__id'])]);
       		  $(".l-grid-tree-link", subParentRow).removeClass("l-grid-tree-link-close").addClass("l-grid-tree-link-open");
       		subParent = g.getParent(subParent);
           	 }
           	 //var allop=$(".l-grid-tree-link");
           //	 $.each(allop,function (i, op){op.hasClass("l-grid-tree-link-open").removeClass("l-grid-tree-link-close");});
        },
        toggle: function (rowParm,loadAjax)
        {
            if (!rowParm) return;
            var g = this, p = this.options;
            var rowdata = g.getRow(rowParm);
            ...
            for (var i = 0, l = children.length; i < l; i++)
            {
                ...
                if (opening)
                {
                 ...
                 // $(".l-grid-tree-link", currentRow).removeClass("l-grid-tree-link-close").addClass("l-grid-tree-link-open"); //liuxianbing_edit_it
                    currentRow.show();
                }
                else
                {
        ...
        _showData: function ()
        {
            ...
            //if (!data) return; liuxianbing_edit
        ...
        _onClick: function (e)
        {
            ...
            g.collapsedRows = g.collapsedRows || [];
			  if (linkbtn.hasClass("l-grid-tree-link-close")) //收缩
            {
			  g.trigger('expandRow',src.data);
			}else{
			  g.trigger('collapseRow',src.data); 
			}
