(function ($)
{
    ...
    LG.showAjaxError = function(xhr,status,errMsg){
    	 var tit="发生错误,错误编码:"+xhr.status;
	   	  if(xhr.status=="0")tit="连接服务器失败!";
	   	  if(xhr.status=="500")tit="内部处理错误!";
	   	  if(xhr.status=="404")tit="404,您懂的!";
	   	  LG.showError(tit);
    };
    LG.showServerError = function(json){
    	 var issuser = json.errorDesc || json.errordesc  ;
        issuser = issuser.split(",");
        var iss="";
        for(var i=0;i<issuser.length;i++){
        	if(i==issuser.length-1)
        	iss+=issuser[i];
        	else iss+=issuser[i]+"_";
        		
        }
        var errormes="";
        if(json.errorCode===undefined || json.errorCode==null|| json.errorCode==""){
        	 errormes="错误原因:"+iss;
        }else{
        	 errormes="错误代码:"+json.errorCode+"<br /><br />"+"错误原因:"+iss;
        }
        LG.showErrorContent(errormes);
        // $.ligerDialog.error(errormes, '操作失败');
    };
    //显示成功提示窗口
    ...
    //显示自定义高度失败提示窗口
    LG.showErrorContent = function (message, callback)
    {
        if (typeof (message) == "function" || arguments.length == 0)
        {
            callback = message;
        ...
        $.ligerDialog.errorTab(message, '提示信息', callback,300,200);
    };
    //显示失败提示窗口
    ...
    LG.bulidFilterGroup = function (form)
    {
        ...
        {
        	var type = $(this).attr("vt") || "java.lang.String";
        	var myval = $(this).children().get(0);
        	var value=$(myval).val();
        	var name = $(myval)[0].name;
        	value =LG.ToCDB(value);
        	if(value!=""){
        		var myop = $(this).parent().find("select").first().val()
        	     group.rules.push({
                     op: myop,
                     field: name,
                     value: value,
  ...
  //搜索条件
    LG.searchForm=function(form, grid){
    	  if (!form) return;
    	  var rule = LG.bulidFilterGroup(form);
           if (rule.rules.length)
           {
    ...
    LG.setFormBtnArray = function (cancleCallback, savedCallback)
    {
        //表单底部按钮
        var buttons = [];
        if (cancleCallback)
        {
        	var tex= cancleCallback[1] || "取消"
            buttons.push({ text: tex, onclick: cancleCallback[0] });
        }
        if (savedCallback)
        {
        	var gen= savedCallback[1] || "更新"
            buttons.push({ text: gen, onclick: savedCallback[0] });
        }
        LG.addFormButtons(buttons);
    };
